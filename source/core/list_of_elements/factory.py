...
class ListOfElementsFactory:
    ...
    def subset_list_run(
        ...
        input_beam = initial_beam_factory.factory_subset(
            simulation_output, get_kw)
...
def new_list_of_elements(
        ...
        instructions_factory: InstructionsFactory | None = None,
        **kwargs: Any,
) -> ListOfElements:
    """
    Create a new :class:`.ListOfElements`, encompassing a full linac.
    ...
    if instructions_factory is None:
        instructions = _dat_filepath_to_plain_list_of_elements(files)
    else:
        dat_filecontent = files['dat_content']
        instructions = instructions_factory.run(dat_filecontent)
    elts = [elt for elt in instructions if isinstance(elt, Element)]
