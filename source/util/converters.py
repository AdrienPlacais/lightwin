...
def envelope_pos(envelope_pos_orig: np.ndarray, gamma: float | np.ndarray,
                 key: str, lam: float | np.ndarray | None = None,
                 e_0: float | np.ndarray | None = None) -> np.ndarray:
    """Convert position envelope array from a phase space to another."""
    if lam is None:
        lam = con.LAMBDA_BUNCH
    if e_0 is None:
        e_0 = con.E_REST_MEV
    beta = np.sqrt(1. - gamma**-2)
    k_1 = 2. * np.pi / (beta * lam)
    conversion_constants = {
        "phiw to z": k_1,
        "z to phiw": 1. / k_1,
        "phiw to zdelta": k_1,
        "zdelta to phiw": 1. / k_1,
        "z to zdelta": 1.,
        "zdelta to z": 1.,
    }
    envelope_pos = envelope_pos_orig * conversion_constants[key]
    return envelope_pos


def envelope_energy(envelope_energy_orig: np.ndarray, gamma: float | np.ndarray,
                    key: str, lam: float | np.ndarray | None = None,
                    e_0: float | np.ndarray | None = None) -> np.ndarray:
    """Convert energy envelope array from a phase space to another."""
    if lam is None:
        lam = con.LAMBDA_BUNCH
    if e_0 is None:
        e_0 = con.E_REST_MEV
    ...
    conversion_constants = {
        "phiw to z": 1e-6 * k_3,
        "z to phiw": 1e6 / k_3,
        "phiw to zdelta": 1e-5 * k_2,
        "zdelta to phiw": 1e5 / k_2,
        "z to zdelta": 1e1 * gamma**-2,
        "zdelta to z": 1e-1 * gamma**2,
    }
    envelope_energy = envelope_energy_orig * conversion_constants[key]
